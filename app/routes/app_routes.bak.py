...
def subdom_enum():
    if request.method == 'POST':
        ...
        # Extract unique subdomains and IPs from the consolidated output
        unique_subdomains, unique_ips = extract_unique_subdomains_ips(consolidated_results)

        result_id = generate_unique_id()

        # Store consolidated results in the database
        txt_path, pdf_path, csv_path = save_results_to_txt(result_id, domain, unique_subdomains, unique_ips)
...
def run_enumeration_script(tool, domain):
    ...
    return result.stdout.strip()  # Return the captured output as a string
...
def show_results(result_id):
    ...
    with open(txt_path, 'wb') as txt_file:  # Open in binary write mode
        txt_file.write(txt_data)  # Write bytes data directly

    with open(txt_path, 'rb') as txt_file:  # Open in binary read mode
        consolidated_results = txt_file.read().decode()  # Read bytes and decode to string

    unique_subdomains = extract_unique_urls(consolidated_results)
    unique_ips = extract_unique_ips(consolidated_results)
...
# Function to extract unique subdomains and IPs from consolidated temporary output
def extract_unique_subdomains_ips(consolidated_results):
    subdomains = extract_unique_urls(consolidated_results)
    ips = extract_unique_ips(consolidated_results)
...
def save_results_to_txt(result_id, domain, unique_subdomains, unique_ips):
    upload_folder = app_routes_bp.config['UPLOAD_FOLDER']  # Access the config through the blueprint
    txt_filename = generate_file_name(domain, 'txt')
    txt_path = os.path.join(upload_folder, txt_filename)

    # Combine unique subdomains and IPs into consolidated results
    consolidated_results = "\n".join(unique_subdomains + unique_ips)
